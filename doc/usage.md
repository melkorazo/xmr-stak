...
```
docker run --rm -it -u $(id -u):$(id -g) --name melkorazo/xmr-stak -v "$PWD":/mnt xmr-stak
docker stop xmr-stak
docker run --rm -it -u $(id -u):$(id -g) --name melkorazo/xmr-stak -v "$PWD":/mnt xmr-stak --config config.txt
```

Debug the docker image by getting inside:

```
docker run --entrypoint=/bin/bash --rm -it -u $(id -u):$(id -g) --name melkorazo/xmr-stak -v "$PWD":/mnt xmr-stak
```
